...
- 다른 PC 등록 명령은 서버 URL과 Site token만 포함하고, GitHub의 `install-connector.ps1`를 받아 나머지 설치/등록을 처리합니다. 대상 PC의 `$HOME\deskrelay`가 없거나, git 저장소가 아니거나, remote가 다르거나, 로컬 변경으로 업데이트할 수 없으면 기존 폴더를 백업하고 새로 clone합니다.
