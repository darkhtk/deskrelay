...
async function openWindowsFileManager(cwd: string): Promise<void> {
  const script = [
    ...
    "    $foreground = [WindowTools]::GetForegroundWindow()",
    "    $currentThread = [WindowTools]::GetCurrentThreadId()",
    "    $foregroundThread = [WindowTools]::GetWindowThreadProcessId($foreground, [IntPtr]::Zero)",
    "    $targetThread = [WindowTools]::GetWindowThreadProcessId($hwnd, [IntPtr]::Zero)",
    "    $attachedForeground = $false",
    "    $attachedTarget = $false",
    "    if ($foregroundThread -and $foregroundThread -ne $currentThread) {",
    "      $attachedForeground = [WindowTools]::AttachThreadInput($currentThread, $foregroundThread, $true)",
    "    }",
    "    if ($targetThread -and $targetThread -ne $currentThread) {",
    "      $attachedTarget = [WindowTools]::AttachThreadInput($currentThread, $targetThread, $true)",
    "    }",
    "    try {",
    "      [WindowTools]::ShowWindowAsync($hwnd, 9) | Out-Null",
    "      [WindowTools]::BringWindowToTop($hwnd) | Out-Null",
    "      [WindowTools]::SetActiveWindow($hwnd) | Out-Null",
    "      [WindowTools]::SetFocus($hwnd) | Out-Null",
    "      [WindowTools]::SetForegroundWindow($hwnd) | Out-Null",
    "      $wshell.AppActivate([int]$match.HWND) | Out-Null",
    "    } finally {",
    "      if ($attachedTarget) { [WindowTools]::AttachThreadInput($currentThread, $targetThread, $false) | Out-Null }",
    "      if ($attachedForeground) { [WindowTools]::AttachThreadInput($currentThread, $foregroundThread, $false) | Out-Null }",
    "    }",
    "    break",
    "  }",
