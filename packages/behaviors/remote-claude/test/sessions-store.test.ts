...
function localCommandCaveat(command: string): string {
  return [
...
describe("listSessions", () => {
  ...
  test("deletes internal local command-only context sessions from the user list", async () => {
    const exactCwd = String.raw`C:\Users\darkh\Projects\deskrelay`;
    const projectDir = join(projectsDir, encodeCwdAsDirName(exactCwd));
    const internalPath = join(projectDir, "internal-context.jsonl");
    const normalPath = join(projectDir, "normal-chat.jsonl");
    await mkdir(projectDir, { recursive: true });
    await writeJsonl(internalPath, [
      { type: "system", subtype: "init", cwd: exactCwd },
      {
        type: "user",
        cwd: exactCwd,
        message: { role: "user", content: localCommandCaveat("/context") },
      },
      { type: "assistant", message: { role: "assistant", content: "Context left: 90%" } },
    ]);
    await writeJsonl(normalPath, [
      { type: "user", cwd: exactCwd, message: { role: "user", content: "normal chat" } },
    ]);

    const result = await listSessions({ projectsDir });
    expect(result.map((s) => s.sessionId)).toEqual(["normal-chat"]);
    expect(await fileExists(internalPath)).toBe(false);
    expect(await fileExists(normalPath)).toBe(true);
  });

  test("keeps sessions that have a real user message after an internal local command", async () => {
    const exactCwd = String.raw`C:\Users\darkh\Projects\deskrelay`;
    const projectDir = join(projectsDir, encodeCwdAsDirName(exactCwd));
    const path = join(projectDir, "mixed-chat.jsonl");
    await mkdir(projectDir, { recursive: true });
    await writeJsonl(path, [
      {
        type: "user",
        cwd: exactCwd,
        message: { role: "user", content: localCommandCaveat("/usage") },
      },
      { type: "user", cwd: exactCwd, message: { role: "user", content: "real follow-up" } },
    ]);

    const result = await listSessions({ projectsDir });
    expect(result.map((s) => s.sessionId)).toEqual(["mixed-chat"]);
    expect(result[0]?.title).toBe("real follow-up");
