...
## 추가 개선 계획: 관리자 대화 출력 정리

목표: 관리자 대화는 사용자가 읽어야 하는 판단과 요약만 남기고, 반복적인 감시/폴링/내부 이벤트는 작업판 상태나 상세 로그로 분리한다.

1. 관리자 대화 메시지 형태 정리
   - 관리자 대화 메시지를 라운드 박스/카드처럼 과하게 감싸지 않는다.
   - 일반 AI 채팅처럼 자연스러운 본문 흐름으로 표시한다.
   - 일반 채팅창은 사용자가 직접 요청하지 않는 한 변경하지 않는다.

2. 폴링/감시 로그의 대화 출력 제거
   - 폴링 횟수, health check, worker heartbeat, 내부 감시 이벤트를 메시지로 누적하지 않는다.
   - 반복 이벤트는 대화가 아니라 작업 탭의 실행 기록 또는 디버그 상세 영역으로 보낸다.
   - 기본 화면에서는 내부 로그를 접거나 숨긴다.

3. 진행 중 상태는 최신 상태 하나만 표시
   - 진행 중에는 `생각 중`, `작업 확인 중`, `worker 응답 대기`, `승인 대기`처럼 현재 행동만 표시한다.
   - 새 상태가 도착하면 이전 진행 상태를 대체한다.
   - 상태 표시는 관리자 컴포저 주변 또는 작업 탭 상태 영역에서 처리하고, 대화 메시지로 쌓지 않는다.

4. 완료 후 요약만 대화에 남김
   - 작업이 끝나면 누적 로그 대신 사용자가 알아야 할 결과 요약만 남긴다.
   - 요약에는 수행한 일, 확인된 결과, 남은 선택지, 필요한 승인만 포함한다.
   - 사용자가 알 필요 없는 내부 retry, polling, stale cleanup 과정은 기본 요약에서 제외한다.

5. 메시지 분류 규칙 추가
   - 관리자 출력은 `user-visible summary`, `live status`, `internal event`, `debug log`로 분류한다.
   - 관리자 대화에는 `user-visible summary`만 누적한다.
   - `live status`는 단일 상태 슬롯에 표시하고, `internal event`와 `debug log`는 작업 탭 상세 로그로 보낸다.

6. 검증 시나리오
   - 긴 관리자 작업을 지시해도 대화창에 폴링 로그가 반복 누적되지 않아야 한다.
   - 진행 중에는 최신 상태만 갱신되어야 한다.
   - 완료 후에는 요약 메시지 하나만 남아야 한다.
   - 상세 로그가 필요한 경우 작업 탭에서 별도로 확인 가능해야 한다.
