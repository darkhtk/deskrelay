...
- 관리자 대화 일부 정리
  - 관리자/사용자 라벨 제거.
  - markdown 렌더링 개선.
  - 일부 내부 notification, timeout chatter, healthz류 노이즈 필터링.
...
- 관리자 대화 응답 표시
  - 응답이 즉시 보이지 않고 다음 지시 때 한꺼번에 보이는 현상이 있었음.
  - 진행 중 상태 표시와 최종 응답 표시의 분리가 더 필요함.
...
- 관리자 대화 출력 UX
  - 아직 메시지가 라운드 박스처럼 보이고, 폴링/감시성 메시지가 대화에 쌓이는 문제가 남아 있음.
...
- timeout 문구를 그대로 노출하지 않고, 살아 있는 작업이면 `worker 응답 대기`로 유지한다.
